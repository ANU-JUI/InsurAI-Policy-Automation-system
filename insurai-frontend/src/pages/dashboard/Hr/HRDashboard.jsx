...
export default function HRDashboard() {
  ...
  const fetchEmployees = useCallback(async () => {
    try {
      ...
      const response = await fetch("https://insurai-policy-automation-system-backend.onrender.com/auth/employees");
  ...
  const fetchHRList = useCallback(async () => {
    try {
      const response = await fetch("https://insurai-policy-automation-system-backend.onrender.com/hr");
  ...
  const fetchPolicies = useCallback(async () => {
    try {
      ...
      const response = await fetch("https://insurai-policy-automation-system-backend.onrender.com/employee/policies", {
  ...
  const fetchClaims = useCallback(async () => {
    try {
      ...
      const response = await fetch(`https://insurai-policy-automation-system-backend.onrender.com/hr/claims?hrId=${loggedInHrId}`, {
  ...
  const approveClaim = async (id, remarks) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`https://insurai-policy-automation-system-backend.onrender.com/hr/claims/approve/${id}`, {
        method: "POST",
        headers: { 
  ...
  const rejectClaim = async (id, remarks) => {
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await fetch(`https://insurai-policy-automation-system-backend.onrender.com/hr/claims/reject/${id}`, {
        method: "POST",
        headers: { 
          "Authorization": `Bearer ${token}`,
          "Content-Type": "application/json"
  ...
  const downloadCSV = () => {
    ...
    const rows = displayedClaims.map(c => [
      ...
      c.priority,
      c.daysPending,
      c.documents?.length > 0 ? c.documents.map(d => `https://insurai-policy-automation-system-backend.onrender.com${d}`).join(" | ") : "No documents"
    ]);
  ...
  const renderEnhancedHomeContent = () => {
    ...
    minHeight: "100vh",
    background: "linear-gradient(135deg, #88a1d9ff 0%, #4c61c0ff 100%)",
    padding: "24px",
  }}>
