...
export default function EmployeePolicies({
  ...
  const [calcType, setCalcType] = useState("HEALTH");
  ...
  // -------------------- Calculate Premium (calls backend) --------------------
  const calculatePremium = async () => {
    setCalcError("");
    setEstimatedPremium(null);
    if (!calcAmount || Number(calcAmount) <= 0) {
      setCalcError("Please enter a valid coverage amount");
      return;
    }
    setCalcLoading(true);
    try {
      ...
      if (data && typeof data.estimatedMonthlyPremium !== "undefined") {
        setEstimatedPremium(Number(data.estimatedMonthlyPremium));
      } else {
        setCalcError("Unexpected response from server");
      }
    } catch (e) {
      setCalcError("Could not calculate premium. Try again later.");
      console.error(e);
    } finally {
      ...
      <div className="card mb-4 p-3">
        <div className="d-flex flex-column flex-md-row gap-3 align-items-center">
          <div style={{ minWidth: 220 }}>
            <label className="form-label mb-1">Coverage Amount (₹)</label>
            <input
              type="number"
          ...
          <div style={{ minWidth: 200 }}>
            <label className="form-label mb-1">Policy Type</label>
            <select className="form-select" value={calcType} onChange={(e) => setCalcType(e.target.value)}>
          ...
          <div className="d-flex align-items-end">
            <button className="btn btn-primary" onClick={calculatePremium} disabled={calcLoading}>
              {calcLoading ? (
          ...
          <div className="ms-auto text-end">
            {calcError && <div className="text-danger small">{calcError}</div>}
            {estimatedPremium !== null && (
              <div>
                <div className="small text-muted">Estimated Monthly Premium</div>
                <div className="fw-bold">₹{estimatedPremium.toLocaleString("en-IN")}</div>
              </div>
            )}
          </div>
        </div>
