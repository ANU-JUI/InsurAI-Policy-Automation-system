...
export default function EmployeeNotification({ userDbId, token }) {
  ...
  // ---------------- Detect New Device / Security Alerts ----------------
  const isNewDeviceAlert = (n) => {
    if (!n) return false;
    const text = `${n.title || ""} ${n.message || ""}`.toLowerCase();
    return (
      text.includes("new device") ||
      text.includes("new device/browser") ||
      text.includes("security alert") ||
      text.includes("new device login") ||
      (n.title && n.title.toLowerCase().includes("security")) ||
      (n.title && n.title.toLowerCase().includes("device"))
    );
  };

  const newDeviceAlerts = useMemo(
    () => notifications.filter((n) => !n.readStatus && isNewDeviceAlert(n)),
    [notifications]
  );

  const markSecurityAlertsAsRead = async () => {
    if (newDeviceAlerts.length === 0) return;
    try {
      await Promise.all(
      ...
      setNotifications((prev) =>
        prev.map((n) => (newDeviceAlerts.some((a) => Number(a.id) === Number(n.id)) ? { ...n, readStatus: true } : n))
      );
    } catch (err) {
      console.error(err);
      alert("Failed to mark security alerts as read.");
    }
  };
  ...
  return (
    <div className="container-fluid">
      ...
      {/* New-Device / Security Alert Banner */}
      {newDeviceAlerts.length > 0 && (
        <div className="alert alert-warning d-flex justify-content-between align-items-center">
          <div>
            <i className="bi bi-shield-exclamation-fill me-2"></i>
            <strong>Security alert:</strong> New device login detected ({newDeviceAlerts.length} unread).
          </div>
          <div className="d-flex gap-2">
            <button className="btn btn-sm btn-outline-dark" onClick={() => { setFilter('unread'); window.scrollTo({ top: 0, behavior: 'smooth' }); }}>
              View
            </button>
            <button className="btn btn-sm btn-success" onClick={markSecurityAlertsAsRead}>
              Mark all as read
            </button>
          </div>
        </div>
